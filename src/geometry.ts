...
clone ()
	...
	Rectangle._temp = new Float32Array(2);_unionWith = function(r) // : void
	{
		if(r.isEmpty()) return;
		if(this.isEmpty()) { this.copyFrom(r); return; }
		this._unionWP(r.x, r.y);
		this._unionWP(r.x+r.width, r.y+r.height);
	}_unionWP = function(x, y)	// union with point
	{
	...
	}_unionWL = function(x0,y0,x1,y1)	// union with point
	{
